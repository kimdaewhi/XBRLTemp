...
namespace XBRLAnalysis
{
    public partial class XBRLAnalysis : Form
    {
        ...
        private System.Data.DataTable _dt_test1111 = new System.Data.DataTable();
        ...
        private void btn_InsertFinData_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ds_Item.Tables.Count; i++)
            {
                if (ds_xbrl.Tables[i].TableName == "dart" || ds_xbrl.Tables[i].TableName.Contains("entity") || ds_xbrl.Tables[i].TableName == "ifrs-full")
                {
                    ...
                    ds_Item.Tables[i].Columns.Add("Item_Category", typeof(string));
                    ds_Item.Tables[i].Columns.Add("ItemName_KO", typeof(string));
                    ds_Item.Tables[i].Columns.Add("ItemName_EN", typeof(string));        // 라벨 명칭 추가할 새로운 Column
            ...
            System.Data.DataTable rslt = _mssqlModel.execProcedure("P_XBRL_MAST_ITEM_SET", new DataModule.Param[] { new DataModule.Param("@PXML_MST_ITEM", SqlDbType.Xml, itemXML) }).Tables[0];
            ...
            dt_result.Columns.Add("ROLE_TYPE_CTG", typeof(string));
            ...
            try
            {
                ...
                ds_rpt.Tables["T_RPT_CORP_MAIN"].Rows.Add(sDocId, sCorpCode, sRPtCd, _sDocDate, sStndDt, sRptTitle, sQtyTyp);
                #endregion ******************************************************


                #region ********************* T_RPT_CORP_FIN Data Insert *********************
                ds_LinkBasePre.Tables[1].Columns.Add("Item_Category");
                ds_LinkBasePre.Tables[1].Columns.Add("Namespace");
                for (int i = 0; i < ds_LinkBasePre.Tables[1].Rows.Count; i++)
                {
                    #region Sheet명, Namespace 추가
                    if (ds_LinkBasePre.Tables[1].Rows[i]["preArc_xlink:to"].ToString() != string.Empty)
                    {
                        string sOrgStr = ds_LinkBasePre.Tables[1].Rows[i]["preArc_xlink:to"].ToString().Substring(ds_LinkBasePre.Tables[1].Rows[i]["preArc_xlink:to"].ToString().Length - (ds_LinkBasePre.Tables[1].Rows[i]["preArc_xlink:to"].ToString().Length - 4));
                        string sSheet = sOrgStr.Split(new[] { '_' }, 2)[0];
                ...
                for (int i = 0; i < ds_xbrl.Tables.Count; i++)
                {
                    if (ds_xbrl.Tables[i].TableName == "dart" || ds_xbrl.Tables[i].TableName.Contains("entity") || ds_xbrl.Tables[i].TableName == "ifrs-full")
                    {
                        ...
                        // 조건 : xbrl 시트명 == pre 계정구분 && xbrl 계정코드 == pre 계정코드
                        var qry = from tb_xbrl in ds_xbrl.Tables[i].AsEnumerable()
                                  join tb_pre in ds_LinkBasePre.Tables[1].AsEnumerable() on xbrlTableName equals tb_pre.Field<string>("Item_Category") into dataKey

                                  from tbPresentation in dataKey.DefaultIfEmpty()
                                  where tbPresentation.Field<string>("Namespace") == tb_xbrl.Field<string>("namespace")
                                  select new
                                  {
                                      DOC_CD = sDocId,
                                      ROLE_CD = (tbPresentation == null) ? "" : tbPresentation.Field<string>("presentationLink_xlink:role").Substring(tbPresentation.Field<string>("presentationLink_xlink:role").Length - 7),
                                      ROLE_TYPE_CTG = "",
                                      ...
                                      ITEM_CD = (tbPresentation == null) ? "" : tbPresentation.Field<string>("Namespace"),
                                      ITEM_VAL = tb_xbrl.Field<double>("value")
                                  };

                        foreach (var row in qry)
                        {
                            dt_result.Rows.Add(row.DOC_CD, row.ROLE_CD, row.ROLE_TYPE_CTG, row.YYMM, row.ITEM_CD, row.ITEM_VAL);
                        }
                ...
                var qry2 = from tb_rptCorpFin in dt_rptCorpFin.AsEnumerable()
                           join tb_LinkRole in dt_LinkRole.AsEnumerable() on tb_rptCorpFin.Field<string>("ROLE_CD") equals tb_LinkRole.Field<string>("ROLE_CD") into dataKey
                           from RptCorpFin in dataKey.DefaultIfEmpty()
                           select new
                           {
                               DOC_CD = sDocId,
                               ROLE_CD = tb_rptCorpFin.Field<string>("ROLE_CD"),
                               ROLE_TYPE_CTG = (RptCorpFin == null) ? "" : RptCorpFin.Field<string>("ROLE_TYP_CTG"),
                ...
                {
                    ds_rpt.Tables["T_RPT_CORP_FIN"].Rows.Add(row.DOC_CD, row.ROLE_CD, row.ROLE_TYPE_CTG, row.YYMM, row.ITEM_CD, row.ITEM_VAL);
                }
                #endregion *******************************************************************
        ...
        private void tv_FolderList_NodeMouseDoubleClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            try
            {
                if (e.Node.Text != string.Empty && e.Node.Text.Contains("."))
                {
                    ...
                    // 변환 자동 실행
                    btn_ConvertXbrl_Click(null, null);
                }
            }
                        ...
                        case "link_presentationLink":
                            ...
                            {   
                                ...
                                if(j % 2 == 0)
                                {
                                    ...
                                    ds.Tables[sheetName].Rows[_ord_Pre]["presentationLink_xlink:role"] = sPresentationLink;
                                    ds.Tables[sheetName].Rows[_ord_Pre]["loc_xlink:href"] = pa.ps.loc.Loc_Href;
                                    ds.Tables[sheetName].Rows[_ord_Pre]["loc_xlink:label"] = pa.ps.loc.Loc_Label;
                                    ds.Tables[sheetName].Rows[_ord_Pre]["loc_xlink:type"] = pa.ps.loc.Loc_Type;

                                    ds.Tables[sheetName].Rows[_ord_Pre]["preArc_xlink:type"] = pa.ps.pa.PA_Type;
                                    ds.Tables[sheetName].Rows[_ord_Pre]["preArc_xlink:arcRole"] = pa.ps.pa.PA_ArcRole;
                                    ds.Tables[sheetName].Rows[_ord_Pre]["preArc_xlink:from"] = pa.ps.pa.PA_From;
                                    ds.Tables[sheetName].Rows[_ord_Pre]["preArc_xlink:to"] = pa.ps.pa.PA_To;
